...
test(functionBreakpoint, [ setup(dapipe(SIn, SOut, CIn, COut)),
    ...
    member(event(_, "stopped",   _{description:_,hitBreakpointIds:_,reason:"entry",text:_,threadId:Id}), Events),
    member(event(_, "continued", _{allThreadsContinued:_,threadId:Id}), Events),
    member(event(_, "stopped",   _{description:_,hitBreakpointIds:_,reason:"breakpoint",text:_,threadId:Id}), Events),
