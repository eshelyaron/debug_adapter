...
da_break_hook(BP, Clause, PC, _FR, _BFR, _Expression, Cond, Hit, Hit, Log, Action) :-
    retractall(da_known_breakpoint(BP, _, _, _, _, _, _)),
    asserta(da_known_breakpoint(BP, Clause, PC, Cond, 0, Hit, Log)),
    da_breakpoint_action(Log, Action).

:- det(da_breakpoint_action/2).
da_breakpoint_action(null          , trace).
da_breakpoint_action(string(String), continue) :-
    print_message(trace, String).
...
da_breakpoints_set(path(Path), Req, Res) :-
    ...
    debug(dap(breakpoint), "Loaded path ~w", [Path]),
    foreach(da_breakpoint_path(BP, Path),
            da_breakpoint_delete(BP)),
    debug(dap(breakpoint), "Deleted stale breakpoints", []),
