...
da_hidden_predicate(Goal) :- qualified(Goal, _, UGoal), memberchk(UGoal, [da_debugee(_, _, _, _), da_trace(_, _, _, _), da_tracer_top_level(_, _), da_terminal(_, _, _), toplevel_call(_)]), !.
